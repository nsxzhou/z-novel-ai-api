...
type ArtifactPipeline struct {
	...
	validator       wfnode.ArtifactValidator
	patcher         wfnode.ArtifactJSONPatcher

	graphOnce sync.Once
	graph     compose.Runnable[*wfmodel.ArtifactGenerateInput, *wfmodel.ArtifactGenerateOutput]
	graphErr  error
...
func NewArtifactPipeline(factory workflowport.ChatModelFactory, retrievalEngine *appretrieval.Engine, validator wfnode.ArtifactValidator, patcher wfnode.ArtifactJSONPatcher) *ArtifactPipeline {
...
func (g *ArtifactPipeline) Generate(ctx context.Context, in *wfmodel.ArtifactGenerateInput) (*wfmodel.ArtifactGenerateOutput, error) {
...
func formatArtifactMessages(ctx context.Context, in *wfmodel.ArtifactGenerateInput) ([]*schema.Message, error) {
...
func (g *ArtifactPipeline) formatArtifactPatchMessages(ctx context.Context, in *wfmodel.ArtifactGenerateInput) ([]*schema.Message, error) {
	...
	current = wfnode.TruncateByRunes(current, 60000)

	allowedOps := ""
	allowedPaths := ""
	if g != nil && g.patcher != nil {
		allowedOps = strings.Join(g.patcher.AllowedOps(), ", ")
		allowedPaths = strings.Join(g.patcher.AllowedPaths(in.Type), ", ")
	}
...
type artifactReActState struct {
	In            *wfmodel.ArtifactGenerateInput
...
func (g *ArtifactPipeline) getGraph() (compose.Runnable[*wfmodel.ArtifactGenerateInput, *wfmodel.ArtifactGenerateOutput], error) {
	g.graphOnce.Do(func() {
...
// 并自动调用对应的工具函数。
func (g *ArtifactPipeline) getToolsNode() (*compose.ToolsNode, error) {
...
func (g *ArtifactPipeline) buildGraph(ctx context.Context) (compose.Runnable[*wfmodel.ArtifactGenerateInput, *wfmodel.ArtifactGenerateOutput], error) {
	graph := compose.NewGraph[*wfmodel.ArtifactGenerateInput, *wfmodel.ArtifactGenerateOutput]()
	...
	if err := graph.AddLambdaNode("init", compose.InvokableLambda(func(ctx context.Context, in *wfmodel.ArtifactGenerateInput) (*artifactReActState, error) {
		if in == nil {
			return nil, fmt.Errorf("input is nil")
		}
		if g == nil || g.factory == nil {
		...
		if g != nil && g.patcher != nil && g.patcher.IsEnabled(in) {
			patchMsgs, err = g.formatArtifactPatchMessages(ctx, in)
			if err != nil {
				return nil, err
			}
		...
		ctx = llmctx.WithWorkflowProvider(ctx, "artifact_generate", in.Provider)
		...
		return &artifactReActState{
			...
			Tools:           tools,
			ToolInfos:       toolInfos,
			MaxToolRounds:   wfmodel.DefaultMaxToolRounds, // 防止死循环的最大轮数限制
			MaxRepairRounds: wfmodel.DefaultMaxRepairRounds,
			Mode:            mode,
		}, nil
	...
	if err := graph.AddLambdaNode("model", compose.InvokableLambda(func(ctx context.Context, st *artifactReActState) (*artifactReActState, error) {
		...
		ctx = llmctx.WithWorkflowProvider(ctx, "artifact_generate", st.In.Provider)

		// 尝试生成
		outMsg, err := st.ChatModel.Generate(ctx, st.Messages, g.buildArtifactModelOptions(st.In, true, st.Mode)...)
		...
		if err != nil && wfnode.IsResponseFormatUnsupportedError(err) {
			...
			outMsg, err = st.ChatModel.Generate(ctx, st.Messages, g.buildArtifactModelOptions(st.In, false, st.Mode)...)
		}
		if err != nil {
			return nil, err
		}
	...
	if err := graph.AddLambdaNode("validate", compose.InvokableLambda(func(ctx context.Context, st *artifactReActState) (*artifactReActState, error) {
		...
		rawJSON := strings.TrimSpace(wfnode.ExtractJSONObject(st.LastAssistant.Content))
		...
		case artifactOutputModeJSONPatch:
			if g == nil || g.patcher == nil || g.validator == nil {
				st.ValidateErr = fmt.Errorf("artifact json_patch not configured")
				return st, nil
			}
			patched, err := g.patcher.Apply(ctx, st.In.Type, st.In.CurrentArtifactRaw, rawJSON)
			if err != nil {
				st.ValidateErr = err
				return st, nil
			}
			content, err := g.validator.NormalizeAndValidate(st.In.Type, strings.TrimSpace(string(patched)))
			if err != nil {
				st.ValidateErr = err
				return st, nil
			}
		...
		default:
			if g == nil || g.validator == nil {
				st.ValidateErr = fmt.Errorf("artifact validator not configured")
				return st, nil
			}
			content, err := g.validator.NormalizeAndValidate(st.In.Type, rawJSON)
			if err != nil {
				st.ValidateErr = err
				return st, nil
			}
	...
	if err := graph.AddLambdaNode("repair", compose.InvokableLambda(func(ctx context.Context, st *artifactReActState) (*artifactReActState, error) {
		...
		repairMsg := g.buildArtifactRepairMessage(st.Mode, st.In.Type, st.ValidateErr, st.LastRawJSON)
	...
	// 6. Finalize: 结果封装节点
	// ---------------------------------------------------------------------
	if err := graph.AddLambdaNode("finalize", compose.InvokableLambda(func(ctx context.Context, st *artifactReActState) (*wfmodel.ArtifactGenerateOutput, error) {
		if st == nil || st.In == nil || st.LastAssistant == nil {
			return nil, fmt.Errorf("state is nil")
		}
		if st.ValidateErr != nil {
		...
		return &wfmodel.ArtifactGenerateOutput{
			Type:     st.In.Type,
...
func (g *ArtifactPipeline) buildArtifactModelOptions(in *wfmodel.ArtifactGenerateInput, enableSchema bool, mode artifactOutputMode) []model.Option {
	...
	if enableSchema {
		schemaObj := g.artifactJSONSchemaForMode(in.Type, mode)
		if schemaObj != nil {
...
func (g *ArtifactPipeline) artifactJSONSchemaForMode(t entity.ArtifactType, mode artifactOutputMode) map[string]any {
	if mode == artifactOutputModeJSONPatch {
		return g.artifactJSONPatchSchema(t)
	}
	return artifactJSONSchema(t)
}

func (g *ArtifactPipeline) artifactJSONPatchSchema(t entity.ArtifactType) map[string]any {
	if g == nil || g.patcher == nil {
		return nil
	}
	paths := g.patcher.AllowedPaths(t)
	if len(paths) == 0 {
		return nil
	}
...
func pickArtifactModel(in *wfmodel.ArtifactGenerateInput) string {
	if in == nil {
		return ""
	}
	if strings.TrimSpace(in.Model) != "" {
...
func (g *ArtifactPipeline) buildArtifactRepairMessage(mode artifactOutputMode, t entity.ArtifactType, err error, rawJSON string) string {
	switch mode {
	case artifactOutputModeJSONPatch:
		return g.buildArtifactJSONPatchRepairMessage(t, err, rawJSON)
	default:
...
func buildArtifactFullJSONRepairMessage(t entity.ArtifactType, err error, rawJSON string) string {
	...
	raw = wfnode.TruncateByRunes(raw, 20000)
...
func (g *ArtifactPipeline) buildArtifactJSONPatchRepairMessage(t entity.ArtifactType, err error, rawJSON string) string {
	raw := strings.TrimSpace(rawJSON)
	if raw == "" {
		raw = "[]"
	}
	...
	allowedPaths := ""
	if g != nil && g.patcher != nil {
		allowedPaths = strings.Join(g.patcher.AllowedPaths(t), ", ")
	}
	return fmt.Sprintf(
