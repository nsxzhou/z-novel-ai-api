...
| **章节生成闭环 (ChapterGen)**  | 章节正文生成（Async Jobs + SSE Stream）            | `handler/chapter.go`, `handler/stream.go` |
...
| **增量 Patch (JSON Patch)**    | 对 `novel_foundation/worldview/characters/outline` 支持 JSON Patch 输出（仅顶层 path 白名单），服务端应用后仍落库/返回完整 JSON | `internal/application/story/artifact_generator.go` |
| **上下文滚动摘要 (Redis)**     | 长会话自动压缩历史（summary + recent turns），注入 Prompt 降低 token 成本 | `internal/interfaces/http/handler/conversation.go` |
| **设定冲突扫描**              | 检测新生成内容与已有设定的矛盾并警告（不阻断生成） | `internal/interfaces/http/handler/conversation.go` |
| **多分支创作**                | 构件版本支持多分支（A/B 并行）、分支头列表与版本对比 | `internal/interfaces/http/handler/artifact.go` |
...
- [x] **增量 Patch 模式**：`novel_foundation/worldview/characters/outline` 支持 AI 输出 `JSON Patch`（仅顶层 path 白名单），服务端应用后仍输出完整 JSON。
- [x] **上下文滚动摘要**：使用 Redis 维护 summary + recent turns，超过阈值自动压缩历史并注入 Prompt。

### 第二阶段：质量实验室 (Milestone 4)

- [x] **设定冲突扫描**：检测新生成内容与已有设定的矛盾并警告（默认开启，可通过 `enable_conflict_scan=false` 关闭）。
- [x] **多分支创作**：支持同一构件的 A/B 分支并行、分支头查询与版本对比（非 main 分支默认不自动激活，可通过 `activate=true` 覆盖）。

### 第三阶段：内容生成闭环 (Milestone 5)

- [x] **章节生成**：补齐章节正文的异步生成（Jobs + Redis Streams）与 SSE 流式生成路径。
...
| 章节生成     | `POST /v1/projects/:pid/chapters/generate`         | ✅ 就绪 |
| 章节重生成   | `POST /v1/chapters/:cid/regenerate`                | ✅ 就绪 |
| 章节流式生成 | `GET /v1/chapters/:cid/stream`                     | ✅ 就绪 |
...
- 对 `novel_foundation/worldview` 启用顶层字段 patch；对数组型 `characters/outline` 仅允许替换顶层数组（且必须存在当前版本 JSON）。
...
- ✅ 增量 Patch 模式（JSON Patch）：支持 `novel_foundation/worldview` 顶层字段 + `characters/outline` 顶层数组（add/replace + path 白名单）
...
### 6.11 当前落地状态（2026-01-09）

#### 6.11.1 已完成

- ✅ PromptRegistry（go:embed）：`internal/workflow/prompt/registry.go` + `internal/workflow/prompt/templates/*.txt`（含 `artifact_v2` / `artifact_patch_v1` / `chapter_gen_v1`）
...
- ✅ 章节生成闭环（Async Jobs + SSE）：`internal/application/story/chapter_generator.go` + `internal/interfaces/http/handler/chapter.go` + `internal/interfaces/http/handler/stream.go` + `cmd/job-worker/main.go`
